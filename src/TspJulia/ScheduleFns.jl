...
#! format: off
exponential_cooling_schedule(; T₀::Real, ρ::Real) =
    k -> T₀ * ρ^k

log_cooling_schedule(; T₀::Real, k₀::Real) =
    k -> T₀ * log(k₀) / log(k + k₀)
...
fast_cooling_schedule(; T₀::Real, ρ::Real, n_iter::Int) =
    k -> T₀ * ρ^(k^2 / n_iter)

slow_cooling_schedule(; T₀::Real, ρ::Real, n_iter::Int) =
    k -> T₀ * ρ^(√k / n_iter)
#! format: on
